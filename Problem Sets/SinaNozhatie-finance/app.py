...
def index():
    ...
    portfolio = db.execute(
        "SELECT symbol, SUM(shares) as shares, price FROM history WHERE user_id = :user_id GROUP BY symbol HAVING SUM(shares) > 0",
        user_id=user_id,
    )
...
def buy():
    ...
    if request.method == "POST":
        ...
        db.execute(
            "UPDATE users SET cash = cash - :purchase WHERE id = :user_id",
            purchase=quote["price"] * shares,
            user_id=session["user_id"],
        )
        db.execute(
            "INSERT INTO history (user_id, symbol, shares, price, transacted, date) VALUES (:user_id, :symbol, :shares, :price, 'BUY', datetime('now'))",
            user_id=session["user_id"],
            symbol=quote["symbol"],
            shares=shares,
            price=quote["price"],
...
def history():
    ...
    transactions = db.execute(
        "SELECT * FROM history WHERE user_id = :user_id ORDER BY date DESC",
        user_id=session["user_id"],
    )
...
def sell():
    ...
    if request.method == "POST":
        ...
        stocks = db.execute(
            "SELECT * FROM history WHERE user_id = :user_id AND symbol = :symbol",
            user_id=session["user_id"],
            symbol=symbol,
        )
        if not stocks:
            ...
            if shares > total_shares:
                ...
                db.execute(
                    "INSERT INTO history (user_id, symbol, shares, price, transacted, date) VALUES (:user_id, :symbol, :shares, :price, 'SELL', datetime('now'))",
                    user_id=session["user_id"],
                    symbol=symbol,
                    shares=shares,
